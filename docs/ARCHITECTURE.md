...
| 항목 | 측정값 | 목표 | 비고 |
| --- | --- | --- | --- |
| **룰 평가** (3,000 정책 × 5 필드, 벡터화) | **0.068 ms** | < 1ms | 20필드 환산 ≈ 0.27ms |
| **룰 열배열 메모리** (3,000 정책 × 7 필드) | **41 KB** | < 10MB | 스키마 JSON 제외 |
| **MWIS 솔버 정확성** (정점 ≤12, 20건) | **20/20 완전탐색 일치** | 100% | G4 게이트 기준 충족 |
| **MWIS 정점 30개** (실사용 규모) | **2.1 ms** | < 2,000ms | 정확해 |
| **MWIS 정점 64개** (최악 가정) | **55.8 ms** | < 2,000ms | 정확해, 폴백 불필요 |
| **DB 전량 스캔** (3,000 정책) | **0.53 ms** | — | 배치 스냅샷 빌드용 |
| **DB 물리 크기** (3,000 정책, 스키마JSON 제외) | **1.5 MB / 72 페이지** | < 500MB | Neon Free 여유 충분 |

### 6.2 측정에서 나온 아키텍처적 발견

🔎 **3,000건 전체가 72 페이지(1.5MB)라, 플래너가 GIN 인덱스를 무시하고 Seq Scan(0.67ms)을 선택했다.**

이것은 인덱스 설계 실패가 아니라 **ADR-001을 뒷받침하는 증거**다. 데이터셋 전체가 Postgres 버퍼 한 줌에 들어간다면, 그것을 애플리케이션 RAM에 통째로 올리는 것은 트릭이 아니라 **자연스러운 형상**이다. PRD §7.2가 요구한 "DB 인덱스 기반 후보 축소"는 이 규모에서 오버엔지니어링이며, 인메모리 벡터 연산이 같은 일을 **10배 빠르고 DB 부하 0**으로 해낸다.
...
| 후보 축소 + 룰 평가 | **0.3ms** | 실측 0.068ms × 필드 수 보정 |
| 충족 예상일 계산 | 2ms | 미충족 항목만 |
| 결과 조립 + 근거 인용 부착 | 20ms | JSONB 슬라이스 |
| 직렬화 (orjson + gzip) | 15ms | ~300KB → 40KB |
| **합계** | **~43ms** | 네트워크 제외 |
| 세션 저장 (비동기) | 0ms | BackgroundTask, 응답 블로킹 없음 |

> PRD 요구 p95 5초 대비 **여유 116배**. 0.1 CPU 무료 인스턴스에서 10배 느려져도 목표를 지킨다. 이것이 무료 티어를 고를 수 있는 근거다.
