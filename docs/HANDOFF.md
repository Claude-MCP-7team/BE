...
| 22 | 서류·금액·연락처가 대부분 비어 있다 | 데이터 | API 구조화 필드에 없고 공고문 본문을 읽어야 나온다(A2). `ANTHROPIC_API_KEY` 가 없어 손으로 15건을 썼는데, 게시 23건 중 **서류 3건 · 금액 4건 · 연락처 4건**뿐이다. FE 는 `dept_tel` 을 필수로 그리면 안 된다 (카드 19개가 깨진다). 키가 생기면 20건에 $2 내외로 전부 채워진다 |
