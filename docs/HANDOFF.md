...
| 20 | **전국에 가까운 zipCd 가 '전국'으로 접히면서 제외 지자체가 사라진다** | 코드 | `normalize.py` 는 zipCd 가 100개 이상이면 `["00"]`(전국)으로 접는다. 그런데 전국 목록에서 **몇 개를 뺀** 공고가 있다. 실례: 농식품 바우처(`20260528005400113227`)는 zipCd 가 238개고, 전국(256개)에서 부천·수원·안산·안양·하남·강서·송파 등 18개가 빠져 있다 — 공고문에도 "미추진"으로 적혀 있다. 지금은 **부천 사용자가 ELIGIBLE 로 나온다.** 근거 문구마저 `전국 (zipCd 238개 전 시군구)` 로 틀리게 적힌다. 조용히 틀리는 쪽이라 신고가 안 들어온다. 제안: 배치가 zipCd 합집합으로 '전국'의 기준 집합을 먼저 만들고, 그보다 작으면 접지 말고 명시 목록을 그대로 룰로 쓴다 (레코드 하나만 보면 판단할 수 없어서 `record_to_policy` 혼자서는 못 고친다) |
