...
남은 것은 **재배포와 데이터**다: 새 스냅샷(23건)을 Render 에 올리는 것,
...
| 배포 | `https://be-27y9.onrender.com` (Render, oregon). **실공고 23건** (seed 31건 중 기간이 열린 것). 재배포 필요 — 배포된 이미지는 아직 3건짜리다 |
...
| 테스트 | **702건** — CI `test` 잡이 701건 통과 (DB 통합 포함). 남은 1건은 SDK 가 없을 때만 도는 경로라 `windows-cp949` 잡이 검사한다 |
...
- **데모 데이터도 고정돼 있다** — `data/demo/`. **실공고 5건 + 사용자 2명.**
...
| ~~19~~ | ~~배포 스냅샷이 2026-10-02 에 0건이 된다~~ | 닫힘 (2026-09-24) | 공고 20건을 새로 수집해 seed 를 31건(게시 23)으로 늘렸다. 09-24 **23건** → 10-01 17건 → 11-01 12건 → 12-01 2건. 제출일에 17건이다. **Render 재배포가 남았다** |
...
| 스냅샷 | `GET /v1/meta/snapshot` → **`policy_count: 3`** (2026-09-24 확인). 그 뒤 seed 를 31건으로 늘렸으므로 **재배포하면 23건**이 된다 |
