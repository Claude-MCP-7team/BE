...
제출용 E2E 시나리오(`tests/e2e/`)가 키 없이 돌고, 고정 데모 데이터(`data/demo/`)가 커밋되어 있다.
데모는 **실제 공고**다 (2026-09-24 수집). 시나리오 5(상충 조합)만 실데이터로 재현되지 않는다 — §8 #16.

**BE 는 배포돼 있고 DB·CORS 까지 붙어 있다** (`https://be-27y9.onrender.com`).
FE 의 계약 검증(`check-api.mjs`) 9개 항목이 전부 통과한다 — 브라우저에서 판정부터
세션 저장까지 전 경로가 돈다.

남은 것은 **데이터뿐**이다: 서류·금액·연락처가 대부분 비어 있고(#22), 서류
유효기간 근거가 1/36 이다(#8). 둘 다 공고문 본문이 있어야 채워진다. 자세한 것은 §8.
...
| 배포 | `https://be-27y9.onrender.com` (Render, oregon). **실공고 23건** 라이브 (`20260924T1531Z-23p-28e1b0dba94e`). seed 31건 중 기간이 열린 것 |
| DB | **연결됨**. `ypc-db` Postgres 18, oregon. 세션 저장·조회 동작. 비밀번호는 2026-09-24 에 교체했다 (§8 #18 의 만료일과는 별개) |
| CORS | `https://fe-psi-tan.vercel.app` + 로컬 `:5173`·`:5174`. **배포된 FE 에서 호출된다** (2026-09-24 확인) |
| FE 협업 | [FE#21](https://github.com/Claude-MCP-7team/FE/issues/21) 이 배포·연동 점검의 실시간 창구다. **FE 의 `check:api` 9개 항목이 전부 통과**한다 (2026-09-24) |
| 리모트 이름 | **기계마다 다르다.** `git remote -v` 로 확인할 것 — 이 문서가 `team` 이라고 적어둔 탓에 `origin` 이 팀 저장소인 환경에서 혼선이 있었다 |
| 테스트 | **723건** — CI `test` 잡이 722건 통과 (DB 통합 포함). 남은 1건은 SDK 가 없을 때만 도는 경로라 `windows-cp949` 잡이 검사한다 |
...
`DATABASE_URL` 과 `PROFILE_ENC_KEYS` 가 있으면 701건이 돈다. 없으면 DB 관련 31건이 skip 된다 — **skip 된 걸 통과로 착각하지 말 것.**

**API 키 없이도 전 경로가 돈다.** `data/demo/` 의 고정 공고 5건이 그 바닥을 받친다:

```bash
python -m batch.build_snapshot data/demo/policies.demo.json -o data/snapshot.json
SNAPSHOT_PATH=data/snapshot.json YPC_FIXED_TODAY=2026-09-24 uvicorn app.main:app
```

기준일을 `2026-09-24`(seed 수집일)로 고정하는 게 중요하다. 실공고라 오늘로 두면
날짜가 지날수록 게시 건수가 줄고, 10-03 부터는 빌더가 **0건이라 거부한다** (§8 #19).
`data/demo/profile.demo.json` 과 `profile.future.json` 두 사용자로 판정해 보면
네 가지 판정 상태가 전부 나온다 (§3 '합성 데모 데이터는…').
...
전부 붙어 있고, 배포된 FE(Vercel)가 브라우저에서 전 경로를 호출한다. FE 가
`check:api` 를 완주했다 (2026-09-23, 세션 CRUD 포함 전 항목 통과).

**2026-09-24 로 스냅샷도 실공고로 바뀌었다** (#17 닫힘). 배포 점검 4개 항목이
전부 닫혔고, **BE 가 막고 있는 것은 없다.**

교체하면서 FE 가 고쳐야 하는 게 생겨 [FE#21 에 알렸다](https://github.com/Claude-MCP-7team/FE/issues/21#issuecomment-5813398202):
`policy_id` 가 전부 바뀌었고(`DEMO-*` 하드코딩이 있으면 깨진다), Mock 이 13건으로
다시 녹화됐고, `FUTURE_PASS` 배지가 `judge.future.json` 에서만 나온다. **빈 목록
화면이 새로 필요하다** — 경기도 밖 사용자는 0건이다 (§3 '합성 데모 데이터는…').
...
**2026-09-20 검증(2차까지) 결과가 반영되어 있다.** 36종 중 **23종**의 소요일·수수료를
실제 민원안내 페이지에서 확인했고, **CSV 값이 틀렸던 8행을 고쳤다.**
...
| D007 본인서명사실확인서 | 소요 최대 1 → **0** (2차. 1차에서 로그 내부 모순으로 보류했던 행) |
| D030 등기사항증명서 | 온라인 700 → **1000** — 기존 700원은 '발급'이 아니라 **'열람'** 수수료였다 |
| D020·D022·D023 대학 제증명 | 방문 1000 → **빈칸** (근거 없는 금액이었다) |
...
대학 제증명(D020·D022·D023)의 근거 없던 1,000원을 지우자 두 수수료 칸이 모두 비었고,
`cheapest_fee_krw` 가 **0 을 돌려주기 시작했다** — 화면이 "무료"라고 말하는 것이다.
실제로는 유료인데 국립대는 규칙, 사립대는 학칙에 따라 달라 단일값이 없다. 사용자는
공짜인 줄 알고 갔다가 창구에서 돈을 낸다.

`cheapest_fee_krw` 는 이제 **모르면 `None`** 이다. 합계(`total_document_cost_krw`)는
아는 것만 더하고, 모르는 건수는 `cost_unknown_document_count` 로 따로 나간다 —
합계만 내보내면 그게 전부인 것처럼 보인다.

> 빈칸의 뜻이 둘이라는 점에 주의: `fee_online_krw=None` 은 원래 **'그 채널로는 발급
> 안 됨'** (D007 은 방문만 가능)이고, 둘 다 비면 **'금액 미상'** 이다.
...
| | 뜻 | 현재 |
| --- | --- | --- |
| `verified` | 소요일·수수료를 페이지에서 확인 | 23/36 |
| `validity_grounded` | 유효기간에 근거(법령 조문)가 있다 | **1/36** |
| `is_fully_grounded` | 둘 다 — 화면의 '추정치' 딱지가 떨어지는 조건 | **1/36** (D012) |

`유효기간_근거` 컬럼이 비어 있는 한 화면은 계속 추정치라고 말한다. 소요일만 확인하고
딱지를 떼면 근거 없는 30일이 검증된 값처럼 보인다 — 가장 조용한 종류의 오류다.

**D012 지방세 납세증명서가 첫 사례다** — 지방세징수법 시행령 제7조(발급일부터 30일)를
확보해 36종 중 처음으로 화면에서 확정으로 나간다. 나머지 22행은 소요일만 확인된
상태라 딱지가 그대로 붙어 있다.

→ **다음 사람이 할 일**: 같은 방식으로 나머지 유효기간 근거 확보.
`D011 납세증명서(국세)` 가 바로 다음 후보다 — 지방세와 같은 30일 구조로 보이지만
국세징수법 시행령 원문을 아직 못 봤다 (법제처 페이지 렌더 실패). **추정이지 근거가
아니다.**
...
| 이유 | 행 |
| --- | --- |
| **소요일 근거 미확보** | D020·D022·D023 대학 제증명 — 수수료는 '학교마다 다름'으로 정리됐고, 어디서나민원 수령까지의 1~3일이 아직 근거 없음 |
| 딥링크는 확보, 본문 확인 미완 | D016 건강보험 자격확인서 · D019 4대보험 가입내역(발급 중단 이력 있음 — 가용성 리스크) |
| 공식 근거 페이지 미확보 | D013 원천징수영수증(홈택스 직접) · D031 확정일자 부여현황 |
| 수수료는 확정, 소요일 미확보 | D030 등기사항증명서 — 대법원규칙 제2994호로 금액 확정(발급 1000 / 방문 1200). 소요일 근거만 남았다 |
...
- **배포** — `Dockerfile`·`render.yaml`·CORS 준비됨. `docs/DEPLOY.md` 참고.
  배포·DB·CORS 까지 끝났다 (§8 '배포 완료 상태')
- **E2E 는 있다** — `pytest tests/e2e` 가 시나리오를 돌린다. 발표 대본이기도 하다.
  시나리오 5(상충을 피한 두 조합)는 실데이터로 재현되지 않아, '왜 안 되는지'를
  단언하는 테스트로 바꿔 뒀다 (§8 #16)
- **데모 데이터도 고정돼 있다** — `data/demo/`. **실공고 31건(게시 23) + 사용자 1명.**
  기준일은 `YPC_FIXED_TODAY=2026-09-24` (seed 수집일)
- **⚠️ 발표 전에 배포 목록 건수를 확인할 것** — 실공고라 날짜가 지나면 줄어든다.
  `curl -sS https://be-27y9.onrender.com/v1/meta/snapshot` 의 `policy_count` (§8 #19)
...
### 지역 코드는 레코드 하나만 보고 판단할 수 없다

`normalize.resolve_region` 은 배치 전체의 zipCd 합집합(`code_universe`)을 받는다.
레코드 하나로는 못 푸는 문제가 둘 있었고, 둘 다 **조용히 틀리는** 쪽이었다.

**(a) 전국으로 접으면 제외 지자체가 사라진다.** 예전엔 zipCd 가 100개를 넘으면
`["00"]`(전국)으로 접었다. 그런데 전국 목록에서 **몇 곳을 뺀** 공고가 있다 —
농식품 바우처는 238개고, 빠진 18곳이 공고문에 "미추진"으로 적힌 부천·수원·안산
등이다. 접으면 부천 사용자가 **받을 수 없는 정책을 적격으로 받는다.**

레코드 하나만 보면 238이 전국인지 전국-18인지 알 방법이 없다. 그래서 묶음의
합집합을 기준으로 삼고, **그걸 전부 덮을 때만** 접는다.

**(b) 시 단위로 입력한 사용자가 구 단위 공고를 못 본다.** API 는 용인을
`41461·41463·41465`(처인·기흥·수지)로 적는다. 사용자가 `41460`(용인시)으로
입력하면 `region_chain` 이 `['00','41','41460']` 이라 교집합이 비고, 정책이
**목록에서 그냥 없어진다.** 부적격도 확인필요도 아니라 화면에 아무 흔적이 없다 —
이 저장소가 가장 경계하는 실패다. 그래서 한 시의 구가 전부 있으면 시 코드도 넣는다.

합집합에서 **5자리가 아닌 코드는 뺀다.** 광역 공고는 zipCd 에 `41`(경기) 처럼
2자리를 적는데, 그게 섞이면 전국 목록(시군구만 열거)이 합집합을 못 덮어서 **모든
전국 공고가 '전국 아님'이 된다.** 같은 이유로 비교 전에 양쪽 다 시 코드까지
펼친다 — 같은 장소를 시(`41590` 화성시)로 적은 공고와 구(`41591`…)로 적은 공고가
섞여 있다.

**판정용과 표시용을 나눈다.** 판정 룰은 250여 개 명시 목록을 쓰고,
`meta.region_code` 는 접은 값을 쓴다. 둘 다 명시 목록으로 하면 목록 응답 한
페이지가 40KB 씩 불어난다. 목록은 넓게 걸러 보여주고(사용자는 카드를 본 뒤
판정에서 정확한 사유를 받는다), 정확한 제외는 판정이 한다.

**기준 집합이 없으면**(단위 테스트처럼 레코드 하나만 부를 때) 개수로 판단하고,
구가 하나라도 있으면 시 코드를 넣는다. 처인구 전용 공고에 기흥구 사용자가
딸려오는 과잉 포함이 생기지만, 정책이 사라지는 것보다 낫다. 근거 문구에
'기준 집합 없음'이라고 남겨서 근사값임이 화면에서도 보인다.

### 모르는 값을 0 으로 내보내면 '무료·즉시'라는 확정값이 된다

하루에 세 번 같은 자리에서 걸렸다. 전부 **빈 값이 숫자 0 으로 바뀌면서** 화면에서
"확인된 사실"로 읽힌 경우다.

| 어디 | 무엇이 새어나갔나 |
| --- | --- |
| `documents.cheapest_fee_krw` | 온라인·방문 수수료가 둘 다 비면 `0` 을 돌려줘서 **유료 서류가 '무료'** 로 보였다 |
| 서류 마스터 로더 | `소요영업일_최소` 가 비면 `0` 이 되고, 마스터 값이라 `lead_time_estimated=False` 라 **'즉시 발급' 확정값**으로 나갔다 |
| `total_document_cost_krw` | 금액 미상 서류만 있으면 합계가 `0` 이라 FE 가 **"예상 발급 비용 0원"** 으로 그렸다 (FE#21, FE PR #26) |

**`0` 과 '모름'은 다른 값이다.** 0 은 "무료다/즉시다"라는 주장이고, 모름은 주장이
없는 상태다. 타입이 `int` 면 둘을 구별할 수 없으므로:

- 모르면 `None` 을 돌려준다 (`cheapest_fee_krw`, 서류별 `cost_krw`).
- 합계처럼 `None` 을 못 쓰는 자리는 **아는 것만 더하고, 모르는 개수를 따로 센다**
  (`cost_unknown_document_count`). 화면이 두 값을 같이 봐야 한다는 뜻이고, 그건
  계약에 적어야 한다 — FE 가 합계만 보고 있었다.
- 입력 쪽에서 아예 못 들어오게 한다 (마스터 로더가 빈 칸을 거부한다). 진짜 0 이면
  `0` 이라고 적으면 된다.

**이 실수는 틀린 답 쪽이 아니라 '틀린 확신' 쪽이다.** 값 자체는 그럴듯해서 아무도
의심하지 않는다. 숫자 필드를 새로 만들 때 "이 필드에 '모름'이 있을 수 있는가"를
먼저 묻는 편이 낫다.

### 원문 링크는 모든 화면이 같은 값을 내야 한다

`/v1/judge` 가 `origin_url or announcement_url`, `/v1/plan` 이 그 반대 순서를 쓰고
있었다. 같은 정책인데 판정 화면과 일정 화면에 **다른 링크**가 걸렸고, 게시 23건 중
8건이 실제로 달랐다. FE 계약 검증에 한 건이 걸려서야 드러났다 (FE#21).

어느 쪽이 맞는지는 아무도 확인하지 않는다 — 둘 다 그럴듯한 정부 사이트라서. 그래서
`app/schemas/policy.py` 의 `public_url()` 하나로 모았고, E2E 가 세 화면을 대조한다.

링크에는 **스킴도 있어야 한다.** 공고 원본에 `www.work.go.kr` 처럼 스킴 없는 주소가
섞여 있는데, 그대로 `<a href>` 에 들어가면 브라우저가 상대 경로로 읽어 **FE 도메인
안의 엉뚱한 페이지**로 간다. 깨진 링크가 아니라 그럴듯한 오동작이라 더 늦게 발견된다.
수집기가 `origin_url` 은 거르고 있었는데 `announcement_url` 은 안 걸러서 새어나갔다.

### 합성 데모 데이터는 실데이터가 깨지는 방식을 감춘다

`data/demo/` 가 2026-09-24 까지 지어낸 공고 5건이었다. 실공고로 갈아 끼우자
**여섯 달 동안 아무도 못 보던 것 세 가지**가 한 번에 드러났다. 셋 다 코드 버그가
아니라 '합성 데이터라서 안 나오던 경우'다.

1. **한 사용자가 네 배지를 다 볼 수 없다.** 실공고는 지역이 서로 배타적이다
   (포천 `41650` · 가평 `41820` · 경기 `41`). 한 사람이 동시에 닿는 공고는 자기
   시·군 것과 광역 것, 최대 두 건인데 판정 상태는 네 가지다. 합성 공고는 지역을
   겹치게 지어서 한 명으로 다 맞출 수 있었다. 지금은 사용자가 둘이다
   (`profile.demo.json` · `profile.future.json`).

2. **경기도 밖 사용자는 목록이 0건이다.** 게시 중인 3건이 전부 경기도 공고고,
   유일한 전국 공고(국토부 청년월세)가 5월에 마감됐다. 합성 데이터에는 전국 공고
   2건이 항상 잡혀 있어서 **빈 목록 화면이 한 번도 필요하지 않았다.** 필터 버그가
   아니라 데이터의 사실이다 — `tests/e2e` 가 0건을 단언하고 이유를 적어 뒀다.

3. **Mock 녹화기가 빌더를 건너뛰고 있었다.** seed 를 그대로 적재해서 마감 필터를
   지나지 않았는데, 합성 5건이 전부 게시 중이라 결과가 같았다. 실공고에는 마감된
   게 섞여 있어서, 그대로 뒀으면 FE 는 **실제로는 오지 않는 공고**로 화면을
   만들었을 것이다.

교훈은 "합성 데이터를 쓰지 마라"가 아니다. 합성 데이터는 **모든 경우가 고르게
들어 있도록 지어지기 때문에**, 실데이터의 특징인 *쏠림과 결손*을 재현하지 않는다.
그래서 빈 목록·단일 지역·전부 마감 같은 경계가 화면에서 한 번도 안 그려진다.
데모용으로 쓰더라도 **최소 한 번은 실데이터를 통과시켜 봐야** 한다.
...
data/
├─ demo/              🟢 고정 데모 — **실공고 31건**(2026-09-24 수집, 게시 23) + 사용자 1명
...
├─ record_mock_responses.py  FE Mock 13건 녹화 (§4 FE Mock)
...
데모 데이터로 실제 API 를 호출해 받은 응답 13건을 커밋해 뒀다. FE 가 BE 없이 화면을
만들 수 있다. `docs/contracts/` 는 스키마지 인스턴스가 아니라서, 필드가 실제로 어떤
값으로 채워지는지(빈 배열인지 `null` 인지, 날짜 형식이 무엇인지)를 알 수 없다.

에러 4종도 녹화돼 있다 — **에러에도 화면이 있다.**

**네 가지 판정 상태가 두 파일에 나뉘어 있다.** seed 가 실공고라 지역이 서로
배타적이어서(포천·가평·경기), 한 사용자가 동시에 닿는 공고가 최대 두 건이다:

| 파일 | 프로필 | 판정 |
| --- | --- | --- |
| `judge.all.json` | 가평 24세, 소득 미입력 | 적격 1 · 확인필요 1 · 부적격 1 |
| `judge.future.json` | 같은 사람 23세 | 적격 1 · **충족예상일 1** · 부적격 2 |

`combinations.json` 과 `plan.json` 은 **역질문에 답한 뒤**로 녹화했다. 미답변
상태로 부르면 적격이 1건뿐이라 조합에 담길 게 없고 서류 배열이 통째로 비어서,
FE 가 서류 화면을 한 번도 못 그려 본 채 제출하게 된다.
...
녹화기는 seed 를 **빌더에 통과시킨 뒤** 적재한다. 처음엔 그냥 적재했는데, 합성
데이터는 5건이 전부 게시 중이라 차이가 드러나지 않았다. 실공고로 바꾸는 순간
마감된 공고까지 Mock 목록에 실릴 참이었다 — 운영과 다른 문을 지나면 FE 는 실제로는
오지 않는 데이터로 화면을 만든다.

기준일은 `2026-09-24` 고정이다 (seed 수집일. `tests/e2e/test_demo_scenarios.py` 의
`DEMO_TODAY` 와 같아야 한다). 나이·충족예상일·신청일정이 오늘 날짜를 타면 매일
...
| ~~20~~ | ~~지역 코드가 조용히 틀린다~~ | 닫힘 (2026-09-24) | 두 가지였다. **(a) 전국 접힘** — zipCd 100개 이상이면 `["00"]` 으로 접어서, 전국에서 18곳을 뺀 농식품 바우처가 부천 사용자에게 ELIGIBLE 로 나갔다. **(b) 시/구 단위 불일치** — API 는 용인을 `41461·41463·41465`(구)로 적는데 사용자가 `41460`(시)으로 입력하면 `region_chain` 교집합이 비어 **정책이 목록에서 통째로 사라졌다**. (b) 가 더 나쁘다 — 부적격도 확인필요도 아니라 화면에 흔적이 없다. 고친 방법은 §3 '지역 코드는 레코드 하나로 판단할 수 없다' 참고 |
| 18 | 무료 Postgres 가 **2026-10-21 에 삭제**된다 | 팀 | 제출(10/1) 이후다. 그 뒤로 데모를 유지하려면 유료 전환이나 재생성이 필요하다 |
| ~~19~~ | ~~배포 스냅샷이 2026-10-02 에 0건이 된다~~ | 닫힘 (2026-09-24) | 공고 20건을 새로 수집해 seed 를 31건(게시 23)으로 늘렸다. 09-25 **23건** → 10-01 17건 → 11-01 12건 → 12-01 2건. 제출일에 17건이고, 배포까지 끝났다 |
| ~~21~~ | ~~`/v1/plan` 의 원문 링크가 다르고 스킴이 없다~~ | 닫힘 (2026-09-24) | FE 계약 검증이 잡아냈다 (FE#21). 엔드포인트마다 `origin_url`/`announcement_url` 우선순위가 반대였고(게시 23건 중 8건이 서로 다른 링크), `announcement_url` 만 스킴 검사를 안 해 `www.work.go.kr` 이 새어나갔다. §3 '원문 링크는 모든 화면이 같은 값을 내야 한다' 참고 |
| 22 | 서류·금액·연락처가 대부분 비어 있다 | 데이터 | 게시 23건 중 **서류 3 · 금액 4 · 연락처 4**뿐이다. FE 는 `dept_tel` 을 필수로 그리면 안 된다 (카드 19개가 깨진다). **`ANTHROPIC_API_KEY` 로는 안 풀린다** — A2 는 API 레코드 텍스트를 읽을 뿐 공고문 페이지를 가져오지 않는다(`--announcements` 로 받아야 한다). 그 텍스트에 애초에 없다: 2026-09-24 수집분 20건의 `*Cn` 필드 평균 **393자**로, 손으로 옮긴 page-001 의 938자·page-002 의 2,568자보다 훨씬 얇다. 원문 텍스트에서 손으로 더 뽑을 수 있는 것도 연락처 1건·금액 2건뿐이다. 실제로 필요한 건 **공고문 페이지 수집**이다 (§2-② 의 크롤러). 그게 되면 키 없이도 손으로 구조화할 수 있고, 키가 있으면 A2 가 대신한다 |
| ~~11~~ | ~~A2 실행용 `ANTHROPIC_API_KEY`~~ | 닫힘 (2026-09-24) | **키를 사도 지금 상황은 안 바뀐다** (#22). 병목이 모델이 아니라 공고문 수집이다. 전수 실행을 하게 되면 아래 비용 추정을 다시 볼 것 |

### 배포 완료 상태 (2026-09-24)

FE 의 배포 점검([FE#21](https://github.com/Claude-MCP-7team/FE/issues/21))이 전부 끝났다.
절차는 전부 `docs/DEPLOY.md` 에 있다.
...
| 스냅샷 | `GET /v1/meta/snapshot` → **`policy_count: 23`** |
| FE 계약 | FE 의 `check-api.mjs` **9/9 통과** — ready · policies · cors-preflight · judge · questions · combinations · plan · 세션 CRUD |
...
**CORS 는 정확히 일치하는 출처만 허용한다** (와일드카드 없음). Vercel 은 배포마다
주소를 새로 발급하므로 **프리뷰 배포에서는 막힌다** — 프로덕션 주소만 등록돼 있다.

```
https://fe-psi-tan.vercel.app                  ← 등록됨
https://fe-psi-tan-git-<브랜치>-<팀>.vercel.app  ← 막힘
```

프리뷰까지 열려면 `allow_origin_regex` 로 바꿔야 하는데, 패턴으로 여는 것은 오타 하나로
의도보다 넓게 열리는 종류라 실제로 필요해질 때 한다.
...
| 15 | FE Mock JSON | `data/demo/responses/` 에 실제 응답 13건을 녹화해 커밋했다 (§4) |
| 9 | FE 공개 배포 주소 → `CORS_ORIGINS` | `https://fe-psi-tan.vercel.app` 등록. preflight 200 + allow-origin 확인 (2026-09-24) |
| 21 | DB 연결 / 세션 저장 | Render Postgres 18(oregon). `/v1/sessions` 201, `/readyz` 의 `database.ready=true` (2026-09-21) |
